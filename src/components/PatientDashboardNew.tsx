...
import { Button, Card, CardContent, CardDescription, CardHeader, CardTitle, Badge, Avatar, AvatarFallback, Alert, AlertDescription, Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, Input, Label, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Switch } from '../ui';
import { 
  User, 
  Calendar, 
  Clock, 
...
function ClinicCard({ 
  ...
  return (
    <Card className={`transition-all duration-200 hover:shadow-lg ${
      clinic.status === 'closed' ? 'opacity-60' : ''
    }`}>
      <CardContent className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-start space-x-4 flex-1">
            <div className="w-16 h-16 bg-teal-100 dark:bg-teal-900 rounded-lg flex items-center justify-center">
              <Building className="w-8 h-8 text-teal-600 dark:text-teal-300" />
            </div>
            <div className="flex-1">
              <div className="flex items-start justify-between">
                ...
                <Button
                  ...
                  className="p-1 h-8 w-8 hover:bg-red-50 dark:hover:bg-red-950"
                >
                  <Heart 
                    className={`w-4 h-4 ${
                      isFavorite 
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium">{clinic.distance.toFixed(1)} km</p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium">{clinic.travelTime} min</p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium">{clinic.rating.toFixed(1)}</p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium">{totalWaiting} patients</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-4">
          <p className="text-xs text-muted-foreground mb-2">Available Doctors ({clinic.doctors.length})</p>
          <div className="flex overflow-x-auto space-x-2 pb-2 scrollbar-hide">
            {clinic.doctors.slice(0, 3).map((doctor) => (
              <div key={doctor.id} className="flex-shrink-0 p-2 bg-muted rounded-lg min-w-[140px]">
                <div className="flex items-center space-x-2">
            ...
            {clinic.doctors.length > 3 && (
              <div className="flex-shrink-0 p-2 bg-muted rounded-lg min-w-[80px] flex items-center justify-center">
                <p className="text-xs text-muted-foreground">+{clinic.doctors.length - 3} more</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between p-3 bg-muted rounded-lg mb-3">
          ...
          <span className="text-sm font-semibold">{avgWaitTime} min</span>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            {clinic.status === 'open' ? 'Open' : clinic.status === 'busy' ? 'Busy' : 'Closed'}
          </Badge>
          <Button
            size="sm"
          ...
          <DialogHeader>
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-4">
                ...
                <div>
                  <DialogTitle className="text-2xl">{selectedClinic?.name}</DialogTitle>
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-muted rounded-lg">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{selectedClinic?.contactNumber}</p>
                </div>
              </div>
              <div className="flex items-center space-x-2">
                <Clock className="w-4 h-4 text-muted-foreground" />
                <div>
                  ...
                  <p className="text-sm font-medium">{selectedClinic?.operatingHours}</p>
                </div>
              </div>
              <div className="flex items-center space-x-2">
                <Star className="w-4 h-4 text-yellow-400 fill-current" />
                <div>
                  <p className="text-xs text-muted-foreground">Rating</p>
                  <p className="text-sm font-medium">{selectedClinic?.rating.toFixed(1)}</p>
                </div>
              </div>
              <div className="flex items-center space-x-2">
                <Car className="w-4 h-4 text-muted-foreground" />
                <div>
                  <p className="text-xs text-muted-foreground">Distance</p>
                  <p className="text-sm font-medium">{selectedClinic?.distance.toFixed(1)} km ({selectedClinic?.travelTime} min)</p>
                </div>
              </div>
            </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex flex-col items-end space-y-2">
                          <div className="text-right">
                            ...
                            <p className="text-sm font-bold">{doctor.queueCount} patients</p>
                            <p className="text-xs text-muted-foreground">~{doctor.avgWaitTime} min wait</p>
                          </div>
                          <Button 
                            size="sm" 
